...
mutate_barcodes <- function(barcodes, mismatches, times) {
  ...
  assert_that(length(unique_width) <= 1L,
              msg=glue("All barcodes must have the same width"))
  assert_that(length(mismatches) == length(times),
              msg = "Arguments mismatches and times must have the same length")
  n_barcodes <- length(barcodes)
  assert_that(sum(times) <= n_barcodes, 
  ...
  sampling_bases <- rep_len(Biostrings::DNA_BASES,
  ...
  for (i in seq_along(mismatches)) {
    ...
    selected_error_positions <- possible_error_positions[,
                                                         x=
                                                           sample(possible_error_positions %>%
                                                                    ncol(),
                                                                  size=times[i],replace = TRUE)]
    # We add a random offset of length 3 to the indicies of the DNA bases
    base_offsets <- sample(
      x = Biostrings::DNA_BASES %>%
        length() %>%
        subtract(1L) %>%
        seq_len(), 
      size = this_mismatches * times[i],
      replace = TRUE
    )
    subscript_matrix <- cbind(selected_error_positions %>% as.vector(),
                              rep(idxs_with_this_mismatches,
                                  each = this_mismatches))
    index_matrix[subscript_matrix] <- 
      index_matrix[subscript_matrix] + 
      base_offsets
  }
  # We could paste everything together,
  # but we choose to optimize the process
