...
Returns an interactive Shiny application for determining cutoff from
knee plot and barcode frequency plot. The user will select the
appropriate number of barcode combinations (i.e. distinct cells) to keep
for further analysis. Usually, this is done by aiming for the 'knee' of
the knee plot in order to keep most reads, while at the same time
removing barcode combinations which are either artifacts or broken
cells.
...
A [`shiny.appobj`](https://rdrr.io/pkg/shiny/man/shinyApp.html) which
launches when printed and returns the last selected cutoff (invisibly)
when it stops.
