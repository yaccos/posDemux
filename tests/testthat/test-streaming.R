...
# name, this is unwanted in most cases
...
callbacks <- streaming_callbacks(
    input_file = input_fastq, output_table_file = output_table_file,
...
expected_filtered_res <- combinatorial_demultiplex(
    sequences = reads, barcodes = barcodes,
    segments = sequence_annotation, segment_lengths = segment_lengths
) %>%
...
test_that("Trying to stream a file where some of the reads are too
    ...
    # This should warn us about seq_3, seq_5, and seq_8 being too short, but not
    # raise an error
    streaming_res <- expect_warning(rlang::exec(streaming_demultiplex, !!!callbacks,
        barcodes = barcodes, allowed_mismatches = 1L, segments = sequence_annotation,
