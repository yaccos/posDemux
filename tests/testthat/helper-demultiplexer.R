...
create_barcode_frame <- function(segment_map, segment_length, n_allowed_mismatches) {
  tibble(
    ...
    mutate(
      length = segment_length %>% magrittr::extract(segment_idx),
      n_allowed_mismatches = n_allowed_mismatches,
      name = glue("bc{1L:3L}")
    ) %>%
    mutate(barcode_reference = pmap(
      list(name, length, n_allowed_mismatches),
      function(name, length, n_allowed_mismatches) {
        DNABarcodes::create.dnabarcodes(
          n = length,
...
create_preliminary_freq_table <- function(possible_barcode_combinations,
  ...
  preliminary_frequency_table <-
    realized_barcode_combinations %>%
    mutate(frequency = ifelse(
      is_artifact,
      rnbinom(n_unique_barcodes,
        mu = mean_reads_per_artifact, size =
          rbinom_size
      ),
      rnbinom(n_unique_barcodes,
        mu = mean_reads_per_cell, size =
          rbinom_size
      )
    ) %>% ifelse(. == 0L, 1L, .)) %>%
    complete_freq_table()
...
test_plot_generation <- function(freq_table) {
  test_that("Frequency and Knee plots are made without raising errors", {
    ...
    suppressMessages(
      ...
      {
        expect_no_error(
          freq_plot(freq_table) %>% ggsave(
            filename = frequency_plot_file,
            plot = .
          )
        )
        expect_no_error(
          knee_plot(freq_table) %>% ggsave(
            filename = knee_plot_file,
            plot = .
          )
        )
      }
...
create_filtered_freq_table <- function(freq_table,
  ...
  filtered_frequency_table <- freq_table %>%
    mutate(frequency = frequency %>% magrittr::inset(
      subtraction_idxs,
      ...
      function(bc_name, i) {
        ...
        list(
          ...
          mismatch_frame = data.frame(
            n_mismatches = 0L:n_allowed_mismatches,
...
test_freq_table <- function(actual, expected, barcode_names) {
  ...
  expect_true(dplyr::setequal(
    actual[columns_to_test],
