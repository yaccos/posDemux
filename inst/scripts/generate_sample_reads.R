# This script generates synthetic example PETRI-seq reads which are
# used for the vignette and the examples
set.seed(59252L)
library(Biostrings)
library(purrr)
library(tibble)
...
concatenate_and_shuffle <- function(barcode_frame) {
  barcode_frame |>
    ...
    xscat(
      umi,
      barcode_index[["bc3"]][bc3],
      bc3_to_bc2_linker,
      barcode_index[["bc2"]][bc2],
      bc2_to_bc1_linker,
      barcode_index[["bc1"]][bc1]
    )
}
...
# Sequences which have barcodes attaches to them, but do
# not have desireable properties
