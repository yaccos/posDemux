...
... and the file containing the forward reads:
...
Note that this example file is a FASTA file. However, in practice
it is usually a FASTQ file. `posDemux` accepts both `DNAStringSet` and `QualityScaledDNAStringSet` as input where the latter contain the quality scores of the reads. However, the quality score is ignored by the demultiplexer and the only reason to pass a `QualityScaledDNAStringSet` is that the quality scores are retained in the payload.

## Running the demultiplexer

Before we run the demultiplexer, we must ensure that the barcodes index is arranged
in the correction order:
...
This can be illustrated by the Knee plot:
...
We continue with the UMIs (remember that the payload and assigned barcodes are aligned in order):
...
With all of this done, we can finally make a data frame containing the UMI, read identifier and barcode assignments:
...
table <- selected_freq_table[,c("bc3","bc2","bc1")]
x <- filtered_res$demultiplex_res$assigned_barcodes %>% as.data.frame()
mapping <- posDemux:::get_mapping(table)
table_encoded <- encode(table, mapping)
x_encoded <- encode(x, mapping)
mask <- x_encoded %in% table_encoded
kept_barcodes <- filtered_res$demultiplex_res$assigned_barcodes[mask,]
# x %in% table
```
```{r}
selected_freq_table[,c("bc3","bc2","bc1")]
S4Vectors::match(filtered_res$demultiplex_res$assigned_barcodes |> as("DataFrame"),
          selected_freq_table[,c("bc3","bc2","bc1")] |> as("DataFrame")
