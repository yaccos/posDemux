...
For this reason, `posDemux` has a streaming API where only parts of the input file
...
The PETRI-seq method is a method for single-cell RNA-sequencing of bacteria `r Citep(bib[c("Blattman2020","Blattman2024")])`. It uses paired-end Illumina sequencing where the forward read contains the cell barcodes and the transcript UMI, whereas the reverse read contain the cDNA to be aligned to the genome. Hence, `posDemux` will only be applied to the forward reads. We start by loading the `posDemux` itself and some helper packages:
...
Hence, we consider the first segment, the UMI, as a payload (denoted as `P` in the sequence annotation) to be kept. The three barcodes (denoted as `B`) are used for demultiplexing. Finally, the linkers and the read past Barcode 1 (denoted as `A`) are ignored.
...
are arranged in descending order on frequency and the most abundant ones are selected.
...
```{r}
reconstrued_bc_cutoff <- freq_to_bc_cutoff(freq_table,
                                           freq_cutoff)
...
res_table <- as.data.frame(selected_assigned_barcodes) %>%
  dplyr::mutate(read = rownames(selected_assigned_barcodes),
...
write.table(res_table, file, row.names = FALSE,
