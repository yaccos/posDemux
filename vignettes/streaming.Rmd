...
The loader is a function which accepts the state as its sole parameter and outputs a list of three parameters:

* `state`: To be passed into the archiver
* `sequences`: The sequences in the chunk as a `Biostrings::DNAStringSet`
...
We decide to hard-code the loader to read in $10^4$ sequences for each chunk.
...
loader <- function(state) {
  n_reads_per_chunk <- 1e4
    ...
    chunk <- Biostrings::readDNAStringSet(filepath = input_fasta, format = "fasta",
                                 nrec = n_reads_per_chunk, skip = state$total_reads)
...
the results of `filter_demultiplex_res()` of the chunk as its second argument.
Its return value is the state to provide to the demultiplexer.

```{r}
output_umi_file <- tempfile(pattern = "umi.fa",
                                 fileext = ".txt")
